...
安全视口是指除去所有 UI 元素及预留缓冲空间后的画布有效展示区域。

由于 `zoomToBounds` API 只能应用均匀的边距 (Inset)，无法满足不对称需求，因此统一设定为 **160px** 的安全边距。

- **统一边距 (Uniform Inset)**: 160px
  - 覆盖顶部导航栏、左侧工具栏、底部输入框及侧边栏展开时的遮挡区域。
